...
import { execAsync, getAbsolutePath, getProperDirName, printTextIntoBox, tryCatchWrapper } from '../../utils/index.js';
import { spinner } from '../../utils/spinner.js';
import { CheckPackageJson, updatePackageJson } from '../manage-package-json/index.js';

import { wantToInstallDepsPrompt, wantToRunProjectPrompt } from '../../prompts/index.js';
import { runProject } from '../run-project.js';

export const initProcess = async ({ name: projectName, ...otherMeta }: Project) => {
...
const installDependencies = async (path: string, otherMeta: Omit<Project, 'name'>) => {
   ...
   await tryCatchWrapper(
      async () => {
         start();
         await execAsync('npm install --force', { cwd: path });
         succeed('Done.');
         printTextIntoBox([

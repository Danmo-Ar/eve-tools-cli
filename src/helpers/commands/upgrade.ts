...
import { getEveCliVersion, logger, printTextIntoBox } from '../../utils/index.js';
...
async function getLatestVersion(isSilent?: boolean): Promise<string> {
   const spinner = isSilent ? undefined : ora(`Fetching latest version for ${PACKAGE_NAME}...`).start();
   try {
...
export async function checkAndUpdateCli(isSilent: boolean = true): Promise<void> {
...
export async function upgradeCli(): Promise<void> {
   ...
   if (semver.gte(currentVersion, latestVersion)) {
      logger.success(`You already have the latest version (${currentVersion}).`);
      return;
   }

   const spinner = ora(`Upgrading ${PACKAGE_NAME} from ${currentVersion} to ${latestVersion}...`).start();
   try {
      await execAsync(`npm install -g ${PACKAGE_NAME}@latest`);
      spinner.succeed(`Successfully upgraded ${PACKAGE_NAME} to ${latestVersion}`);
